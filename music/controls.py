...
class PlayerControls(discord.ui.View):
    ...
    def update_buttons(self):
        ...
        E = {
            ...
            'autoplay': '<:autoplay:1412531621990629466>',
            ...
            if interaction.message and interaction.message.embeds:
                ...
                try:
                    ...
                    for i, field in enumerate(embed.fields):
                        if "<:autoplay:1412531621990629466> Volume" in field.name:
                            embed.set_field_at(i, name="<:autoplay:1412531621990629466> Volume", value=f"```{vol}%```", inline=field.inline)
    ...
    async def pause_resume(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            emoji = "<:play:1412530216965767349>" if not self.player.paused else "<:pause:1412529948861665491>"
            
            await self.update_embed_and_view(interaction)
            
            # Send ephemeral feedback message
            message = await interaction.followup.send(
                embed=discord.Embed(
                    description=f"{emoji} **{action}** the music", 
                    color=discord.Color.blue()
                ), ephemeral=True
            )
            
            # Auto-delete after 3 seconds
            asyncio.create_task(self._auto_delete_message(message, 3))
        ...
        try:
            ...
            if current_track:
                ...
                if res and res.tracks:
                    ...
                    message = await interaction.followup.send(
                        embed=discord.Embed(
                            description=f"<:skip:1412530943121555546> **Skipped** to next track", 
        ...
        try:
            ...
            if current_track:
                ...
                if res and res.tracks:
                    ...
                    message = await interaction.followup.send(
                        embed=discord.Embed(
                            description=f"<:prev:1412530972779352214> **Went back** to previous track", 
                            color=discord.Color.green()
                        ), ephemeral=True
                    )
    ...
    async def vol_down(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"<:autoplay:1412531621990629466> Volume decreased to **{new_vol}%**", 
                color=discord.Color.blue()
            )
    ...
    async def vol_up(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            # Send self-destruct message
            embed = discord.Embed(
                description=f"<:autoplay:1412531621990629466> Volume increased to **{new_vol}%**", 
                color=discord.Color.blue()
            )
            message = await interaction.followup.send(embed=embed, ephemeral=True)
    ...
    async def stop_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            # Send the MomijiWave message first
            embed = discord.Embed(
    ...
    async def loop(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            # Update the embed to reflect the new loop mode
            await self.update_embed_and_view(interaction)
            
            # Send ephemeral feedback message
            loop_modes = {0: "Off", 1: "Track", 2: "Queue"}
            loop_emojis = {0: "➡️", 1: "🔂", 2: "🔁"}
            mode_name = loop_modes.get(cur, "Off")
            emoji = loop_emojis.get(cur, "➡️")
            
            message = await interaction.followup.send(
                embed=discord.Embed(
                    description=f"<:loop:1412531198147952832> Loop mode: **{emoji} {mode_name}**", 
                    color=discord.Color.blue()
                ), ephemeral=True
            )
    ...
    async def shuffle(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            message = await interaction.followup.send(
                embed=discord.Embed(
                    description=f"<:shuffle:1412532183750676532> **Shuffled** the queue", 
                    color=discord.Color.green()
                ), ephemeral=True
            )
    ...
    async def playlist(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            view = QueueView(self.queue_store, guild_id, current_page)
    ...
    async def _auto_delete_message(self, message, delay: int = 3):
        """Helper method to auto-delete ephemeral messages."""
        try:
...
class QueueView(discord.ui.View):
    """Interactive queue view with pagination and clear button."""
    
    def __init__(self, queue_store, guild_id: int, current_page: int = 0):
    ...
    def update_buttons(self):
        ...
        queue = self.queue_store.get_queue(self.guild_id)
        if not queue:
            return
        
        total_pages = max(1, -(-len(queue) // self.items_per_page))
        ...
        # Update Clear button (always enabled if queue exists)
        self.clear_queue.disabled = len(queue) == 0
    
    def get_queue_embed(self):
        """Generate the queue embed for current page."""
        queue = self.queue_store.get_queue(self.guild_id)
        current_index = self.queue_store.get_index(self.guild_id)
        ...
        end_index = min(start_index + self.items_per_page, len(queue))
        ...
        embed = discord.Embed(
            title="<a:Milk10:1399578671941156996> Music Queue", 
            description=queue_list, 
            color=discord.Color.blue()
        )
        embed.set_footer(text=f"Page {self.current_page + 1}/{total_pages} | Total: {len(queue)} | Current: {current_index + 1}")
    ...
    async def next_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            logger.error(f"[QueueView] Error in next_page: {e}")
            await interaction.response.send_message("Error navigating queue.", ephemeral=True)

    @discord.ui.button(label="Clear", emoji="<:trash:1415172903061815317>", style=discord.ButtonStyle.danger, row=0)
    async def clear_queue(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Clear the entire queue."""
        try:
            # Clear the entire queue including current track
            self.queue_store.clear_guild(self.guild_id)
            ...
            logger.info(f"[QueueView] Queue cleared for guild {self.guild_id}")
            
        except Exception as e:
            logger.error(f"[QueueView] Error clearing queue: {e}")
